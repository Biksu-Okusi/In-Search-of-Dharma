...
no narration by design). Each video is a still title frame with the published
narration underneath, a forced-aligned WebVTT caption file so viewers can read
along, and a paste-ready description with chapter timestamps.
...
```
video/
  mk-video.sh        orchestrator: frame → vtt → mp4 → desc, per Part
  ...
  out/
    ...
    N-in-search-of-dharma.mp4       the video                                       [gitignored]
...
1. **Frame** — ImageMagick composes the Part's `<image>` (PNG master in
   `../images/`) at 900×900 on a dark ground with the title, Part line and
   author in EB Garamond. Title text comes from the Part's frontmatter
...
3. **Mux** — ffmpeg loops the frame under the MP3 (libx264 `-tune stillimage`,
   AAC 96 kbps, `+faststart`).
...
./mk-video.sh -s frame   # one step only (frame, vtt, mp4, desc)
...
1. **Create → Upload videos** → `out/N-in-search-of-dharma.mp4`.
