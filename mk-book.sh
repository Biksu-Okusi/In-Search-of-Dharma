...
# Research-note links. Each essay's "Sources & further reading" section links the
# notes it draws on by repo-relative path (../ prefixed in the Indonesian
# edition), which resolves only for someone browsing the repository. Those paths
# are rewritten to absolute GitHub URLs at build time so they resolve from an
...
# That package ships Georg Duffner's unfinished v0.016, whose italic f-ligature
# glyphs (ff, ffi, ffl) are drawn far too heavy and print as ink blots. The
# vendored copy also supplies a true Bold Italic and covers U+221A (the root
...
main() {
  ...
  # and the PDF sheet below. Fonts are embedded under EPUB/fonts/; this CSS
...
/* Oldstyle (non-lining) figures, which is Bona Nova's default and the
   publisher's house style: they sit on the x-height and ride with lowercase
   text instead of standing above it. Asked for explicitly so the setting
   survives a font change. */
body{font-variant-numeric:oldstyle-nums;font-feature-settings:"onum" 1,"liga" 1,"kern" 1}
...
CSS
  ...
  # BUILT holds exactly the files this run produced, in build order. It replaces
  # the former "${OUTPUT%.epub}"* glob, which would sweep up the other typeface
  # sets' artefacts (their names share that stem) as well as anything stale left
  # in the build directory by an earlier run.
