...
Let's begin with the accusation this whole book exists to answer: that a project which privileges no dharma has thereby forfeited the right to judge any, and that "unprivileged" is just relativism under another name.
...
Two cautions come with this test, and both of them from Berlin. The floor identifies failures; it does not rank successes. Above the line the goods genuinely conflict, and where values are incommensurable there will be ties and tragic trade-offs, not a league table. And the judging power is not the imposing power: knowing that a dharma has fallen through the floor tells us where argument, pressure and refusal are justified. It tells us nothing about who may enforce that judgement, or how. Those are different powers, and Part 6's conditions on intervention bind the second, not the first.
...
Run the tests across the cases this project has already put in evidence, and notice that the second test repeatedly sees what the first one squints at.
...
For each of these, the test must convert or confess. For the children it converts into a demand about equipment and exit: a dharma passes only insofar as it gives the child the tools to examine it and the realistic possibility of leaving it, which is exactly what the *wong sikep* schooling case was about, and why its revision mattered so much. For the dead it converts into a denial of tenure: their pen passed to the living at the grave, and a bequest is honoured as an inheritance, not obeyed as a command.
