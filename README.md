...
Producing the printed edition runs on email with the publisher, and [`tools/`](tools/) holds three small scripts for keeping up with it, two of them read-only. [`tools/mailwatch.py`](tools/mailwatch.py) reports messages about the book that have not been seen before, from a Maildir inbox, any extra folders, and the Sent folder when the mail is addressed to a correspondent; silence means no news. [`tools/mailshow.py`](tools/mailshow.py) prints a thread by subject with the quoted tails trimmed, and can save image attachments, which is how a marked-up screenshot of a proof gets looked at. Neither moves or flags a message. [`tools/mailsend.py`](tools/mailsend.py) is the third, and the only one that can do something irreversible, so it is a dry run unless told `--send`: it builds a plain-text reply threaded to the message being answered, and refuses to send a body that does not say who is writing, a recipient who is not a known correspondent, a Markdown or missing attachment, a print interior that is older than its sources or fails preflight, or a message that has already gone out. [`tests/test-mailsend.sh`](tests/test-mailsend.sh) runs it against a fixture mailbox with a stub in place of `sendmail`. [`tools/mk-barcode.py`](tools/mk-barcode.py) is unrelated to mail: it draws the cover barcode for an ISBN, the EAN-13 symbol with its five-digit price add-on, as a vector SVG in pure black with the quiet zones built in, and refuses a number whose check digit is wrong. Where the mailbox lives and whose mail counts are read from a local settings file that is never tracked; [`tools/mail.conf.example`](tools/mail.conf.example) shows its shape.
